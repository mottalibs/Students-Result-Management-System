...
const TeacherDashboard = ({ viewMode = 'dashboard' }) => {
    ...
    const refreshBg = async () => {
        ...
        localStorage.setItem('teacher_dashboard_cache', JSON.stringify({ students: studentsData, results: resultsData, timestamp: Date.now() }));
    };
    ...
    return (
        <div className="space-y-6 max-w-[1400px] mx-auto">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-to-r from-slate-50 to-blue-50 dark:from-slate-800 dark:to-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700">
                <div>
                    ...
                    <h1 className="text-2xl font-bold text-slate-900 dark:text-white">
                        {greeting}, <span className="text-blue-600 dark:text-blue-400">{teacher.name || 'Teacher'}</span>!
                    </h1>
                    <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                        Department: <span className="font-semibold text-slate-700 dark:text-slate-300">{teacher.department || 'General'}</span>
                    </p>
                </div>
                <div className="flex flex-wrap gap-2">
                    <Link to="/teacher/students" className="flex items-center gap-2 px-4 py-2.5 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-200 rounded-xl text-sm font-medium hover:bg-slate-50 dark:hover:bg-slate-600 transition shadow-sm">
                        <UserPlus className="w-4 h-4" /> Add Student
                    </Link>
                    <Link to="/teacher/entry" className="flex items-center gap-2 px-4 py-2.5 bg-blue-600 text-white rounded-xl text-sm font-medium hover:bg-blue-700 transition shadow-sm">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <StatCard label="Total Students" value={stats.totalStudents} icon={Users} link="/teacher/students" accent="blue" />
                <StatCard label="Total Results" value={stats.totalResults} icon={BookOpen} link="/teacher/results" accent="green" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                <ActionCard icon={Eye} title="View Results" desc="Browse all student results" to="/teacher/results" />
                <ActionCard icon={Layers} title="Bulk Upload" desc="Import from Excel/CSV" to="/teacher/bulk-upload" />
                <ActionCard icon={Download} title="Reports" desc="Generate PDF reports" to="/teacher/reports" />
                <ActionCard icon={FileText} title="Add Result" desc="Enter single result" to="/teacher/entry" />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <Link to="/teacher/results" className="text-xs text-blue-600 hover:underline">View All →</Link>
                    </div>
